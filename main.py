...
def analisar_estatisticas(fixture_id, home_team, away_team, minuto, gols_casa, gols_fora, tempo):
    ...
    try:
        ...
        # --- FILTRO DE PRESSÃO ---
        # Só manda o alerta se o jogo tiver pelo menos 4 chutes totais (indica jogo movimentado)
        if chutes_totais < 4:
            print(f"Jogo {home_team} x {away_team} ignorado por falta de pressão ({chutes_totais} chutes).")
            return
        ...
        msg = (
            ...
            f"🎯 *Estratégia:* Minuto ideal com volume de jogo! Buscar +2 cantos no tempo atual."
        )
        enviar_telegram(msg)
        time.sleep(2)
    except Exception as e:
...
def monitorar_jogos_ao_vivo():
    # --- FILTRO DE HORÁRIO SALVA-API (Horário de Brasília) ---
    fuso_br = pytz.timezone('America/Sao_Paulo')
    hora_atual = datetime.now(fuso_br).hour
    
    # Se for antes das 10h da manhã ou depois das 23h da noite, o robô não roda
    if hora_atual < 10 or hora_atual >= 23:
        print(f"Fora do horário comercial (Agora são {hora_atual}h). Economizando API.")
        return

    print("🤖 Monitorando jogos dentro do horário permitido...")
