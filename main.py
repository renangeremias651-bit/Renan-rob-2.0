...
def buscar_odds_pre_jogo(fixture_id):
    """Busca as odds iniciais/pré-jogo para o mercado de gols"""
    ...
    try:
        ...
        if response.get('response'):
            ...
            for book in bookmakers:
                # Procura preferencialmente pela Bet365 ou primeiras listadas
                for bet in book.get('bets', []):
                    if "Goals Over/Under" in bet.get('name', ''):
                        for value in bet.get('values', []):
                            if value.get('value') == '0.5' and 'Over' in value.get('value', ''):
                                return value.get('odd')
    except Exception as e:
        print(f"Erro ao buscar odds: {e}")
    return "Buscando ao vivo..."
...
def analisar_estatisticas(fixture_id, home_team, away_team, minuto, gols_casa, gols_fora, enviados):
    # Trava dupla: Se o ID do jogo já foi alertado hoje, ignora completamente
    if str(fixture_id) in enviados:
        print(f"Jogo {home_team} x {away_team} já foi enviado anteriormente.")
        return
    ...
    try:
        ...
        # Puxa a ODD de referência
        odd_referencia = buscar_odds_pre_jogo(fixture_id)

        msg = (
            f"🔥 **ALERTA DE ENTRADA CONFIRMADO** 🔥\n\n"
            ...
            f"• **Entrada:** Over 0.5 Gols HT (Gol no 1º Tempo)\n"
            f"• **Odd de Referência:** {odd_referencia}\n\n"
            f"🎯 *Estratégia:* Jogo muito truncado com alta tendência de acréscimos longos e aberturas na zaga. Buscar o gol antes do intervalo!"
        )
        
        enviar_telegram(msg)
        salvar_enviado(fixture_id) # Registra no arquivo para nunca mais repetir esse jogo
        time.sleep(2)
...
def monitorar_jogos_ao_vivo():
    ...
    try:
        ...
        for jogo in jogos:
            ...
            # Janela estável: 1º Tempo entre 20 e 30 minutos e placar 0x0
