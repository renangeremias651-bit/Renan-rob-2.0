...
def buscar_odds_pre_jogo(fixture_id, total_gols_atual):
    ...
    linha_alvo = str(total_gols_atual + 0.5)
                ...
                for bet in book.get('bets', []):
                    if "Goals Over/Under" in bet.get('name', ''):
                        for value in bet.get('values', []):
                            if value.get('value') == linha_alvo and 'Over' in value.get('value', ''):
...
def monitorar_jogos_ao_vivo():
    ...
    try:
        ...
        for jogo in jogos:
            ...
            # Janela de análise: 1º Tempo entre 20 e 30 minutos (QUALQUE PLACAR)
            if 20 <= minuto <= 30:
